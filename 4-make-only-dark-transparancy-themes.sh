...
for m in "${arrayname[@]}"; do
	...
	cd arc-theme || exit
	[[ -d build ]] || mkdir build
	meson setup --prefix=/tmp/at -Dthemes=cinnamon,gnome-shell,gtk2,gtk3,gtk4,metacity,plank,unity,xfwm -Dvariants=dark -Dtransparency=true build/
	meson configure --prefix=/tmp/at -Dthemes=cinnamon,gnome-shell,gtk2,gtk3,gtk4,metacity,plank,unity,xfwm -Dvariants=dark -Dtransparency=true build/
